...
class Application_Model_Annonce
{
	...
	public static function getAllAnnonces(){
		...
		foreach ($liste as $item){
			$item['_ville'] = Application_Model_Ville::getVilleById(1 /*$item['_ville']*/);
			$item['_type'] = Application_Model_Type::getTypeById(1 /*$item['_type']*/);
			$item['_categorie'] = Application_Model_Categorie::getCategorieById(1 /*$item['_categorie']*/);
			$item['_annonceur'] = Application_Model_Annonceur::getAnnonceurById(1 /*$item['_annonceur']*/ );
			$item['_photos'] = Application_Model_Photo::getPhotosByIdAnnonce($item['_id_annonce']);
			if( !$item['_ville'] || !$item['_type'] || !$item['_categorie'] || !$item['_annonceur']){	
				$erreur = true;
			}else{
				$annonce = new Application_Model_Annonce;
	...
	public static function getAnnonceById($id){
		$donnees = Application_Model_WebServices_Annonce::getAnnonceById($id);
		if (!$donnees){
			throw new Zend_Controller_Action_Exception('Annonce inexistante', 404);
		}else{
			$donnees['_ville'] = Application_Model_Ville::getVilleById($donnees['_ville']);
			$donnees['_type'] = Application_Model_Type::getTypeById($donnees['_type']);
			$donnees['_categorie'] = Application_Model_Categorie::getCategorieById($donnees['_categorie']);
			$donnees['_annonceur'] = Application_Model_Annonceur::getAnnonceurById($donnees['_annonceur']);
			$donnees['_photos'] = Application_Model_Photo::getPhotosByIdAnnonce($donnees['_id_annonce']);
